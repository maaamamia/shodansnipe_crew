    ...
    def call(self, fn, *args, **kwargs):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                err = str(e).lower()
                # Don't retry auth/plan errors — they won't self-resolve
                if any(x in err for x in ("403", "access denied", "upgrade", "401", "invalid api key")):
                    raise
                last_exc = e
            ...
            if attempt < self.max_retries:
                ...
                logger.warning("RateLimiter: attempt %d failed (%s), retrying in %.1fs", attempt + 1, last_exc, sleep_time)
...
class InternetDBClient:
    ...
    async def query_ip_async(self, ip: str, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        _empty = {"ports": [], "cpes": [], "vulns": [], "hostnames": [], "tags": []}
        async with self.semaphore:
            url = f"{self.BASE_URL}/{ip}"
            try:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    # 404 = IP not in InternetDB — normal, cache it
                    if resp.status == 404:
                        self.cache[ip] = _empty
                        return _empty
                    # 400/429/503 = CDN/rate-limit intercept (e.g. Akamai GHost).
                    # Not a code error — treat as no data, don't log at WARNING level.
                    if resp.status in (400, 429, 503):
                        logger.debug("InternetDB %d for %s — CDN/rate-limit, skipping", resp.status, ip)
                        return _empty
                    if resp.status != 200:
                        logger.debug("InternetDB unexpected status %d for %s", resp.status, ip)
                        return _empty
                    # Check content-type before trying to parse JSON
                    ct = resp.headers.get("content-type", "")
                    if "json" not in ct:
                        logger.debug("InternetDB non-JSON response for %s (ct=%s)", ip, ct)
                        return _empty
                    raw = await resp.json()
                    data = {
                ...
                logger.debug("InternetDB timeout for %s", ip)
                return _empty
            except Exception as e:
                # Truly unexpected — log at DEBUG so it doesn't spam the console
                logger.debug("InternetDB error for %s: %s", ip, repr(e))
                return _empty
...
class ShodanQuery:
    def __init__(self, api_key: str, batch_size: int = 25, max_concurrency: int = 10, thread_pool_size: int = 20):
        ...
        self.rate_limiter = RateLimiter(calls_per_second=3.0, max_retries=3)
    ...
    def _collect_ips(self, query: str, limit: int, tags: Optional[List[str]]) -> tuple[list[str], str | None]:
        ...
        try:
            ...
            # Free key / plan limitation — try fallback to api.search()
            ...
            # Filter plan-restriction errors — user-facing warnings, not code bugs.
            # Shodan returns: 'The "vuln" filter is only available to academic users...'
            elif any(x in err for x in (
                "filter is only available", "requires an api",
                "academic users", "small business", "corporate",
            )):
                logger.warning("Shodan filter restriction: %s", err_orig)
                return [], (
                    f"Shodan plan restriction: {err_orig} — "
                    "Remove the restricted filter or upgrade your Shodan plan."
                )

            else:
                logger.error("Shodan search error: %s", err_orig)
                return [], f"Shodan API error: {err_orig}"
    ...
    async def execute_query(
        ...
        async with aiohttp.ClientSession() as session:
            # Run all batches concurrently - semaphore inside _process_one caps parallelism
            await asyncio.gather(*[self.process_batch(batch, enrich, session) for batch in batches])
